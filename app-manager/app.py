...
identifiants sont aussi recopies dans le .env partage avec codelab-postgres
pour consultation directe depuis le disque du ZimaOS, sans "docker exec".
"""
import json
import os
import re
...
SHARED_ENV_FILE = os.path.join(SHARED_CONFIG_DIR, ".env")
...
def upsert_shared_env(pairs):
    """Ecrit/met a jour des paires CLE=valeur dans le .env partage avec
    codelab-postgres (/DATA/AppData/codelab/config/.env sur le disque du
    ZimaOS). Ne touche qu'aux cles listees ici, laisse celles d'un autre
    service (ex. POSTGRES_*) intactes -- meme logique que le cote
    codelab-postgres dans docker-compose.yml, pour que les deux
    cohabitent sans jamais s'ecraser l'un l'autre."""
    try:
